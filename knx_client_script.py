...
def disconnect_gateway(channel_id, control_endpoint, sock, gateway_ip, gateway_port):
    logger.info('Error ! Trying to disconnect')
    while (1):
        # -> Disconnect Request
        ...
        # <= Disconnect Response
        logger.debug('waiting for a Disconnect response (DISCONNECT_RESPONSE -> disc_resp)')
        data_recv, addr = sock.recvfrom(buf_size)
        disc_resp = knxnet.decode_frame(data_recv)
        logger.info('Disconnect reponse status: {}'.format(disc_resp.status))
        if disc_resp.status == 0:
            logger.info('Disconnected')
            return 1
        logger.info('Error ! Disconnect response status {} is not correct, disconnect'.format(disc_resp.status))
...
def send_knx_request(
    ...
    # we check the status codes first
    if conn_resp.status != 0:
        disconnect_gateway(conn_resp.channel_id, control_endpoint, sock, gateway_ip, gateway_port)

    # Then we send the connection state request
    con_state_req = knxnet.create_frame(
        knxnet.ServiceTypeDescriptor.CONNECTION_STATE_REQUEST,
        conn_resp.channel_id,
        control_endpoint
    )
    logger.debug('>>> CONNECTION_STATE_REQUEST:\n{}'.format(con_state_req))
    sock.sendto(con_state_req.frame, (gateway_ip, gateway_port))

    # <- Connection State Response

    # We get the connection state response status
    ...
    if conn_state_resp.status != 0:
        if disconnect_gateway(conn_state_resp.channel_id, control_endpoint, sock, gateway_ip, gateway_port) == 1:
            return True, 'Done'

    # -> Tunneling Request
    tune_req = knxnet.create_frame(
        knxnet.ServiceTypeDescriptor.TUNNELLING_REQUEST,
        dest_group_addr,
        conn_state_resp.channel_id,
        payload[0],
        payload[1],
        payload[2],
        0x11
    )
    logger.debug('>>> TUNNELLING_REQUEST:\n{}'.format(tune_req))
    sock.sendto(tune_req.frame, (gateway_ip, gateway_port))

    # <- Tunneling Ack
    logger.debug('waiting for a tunneling ack (TUNNELLING_ACK -> tune_ack_resp)')
    data_recv, addr = sock.recvfrom(buf_size)
    tune_ack_resp = knxnet.decode_frame(data_recv)
    logger.info('reponse status: {}'.format(tune_ack_resp.status))

    if tune_ack_resp.status != 0:
        if disconnect_gateway(tune_ack_resp.channel_id, control_endpoint, sock, gateway_ip, gateway_port) == 1:
            return True, 'Done'

    # <- Tunneling Request response
    logger.debug('waiting for a tunneling request resp (TUNNELLING_REQUEST -> tune_req_resp)')
    data_recv, addr = sock.recvfrom(buf_size)
    tune_req_resp = knxnet.decode_frame(data_recv)
    logger.info('reponse data services: {}'.format(tune_req_resp.data_service))
    if tune_req_resp.data_service != 0x2e:
        if disconnect_gateway(tune_ack_resp.channel_id, control_endpoint, sock, gateway_ip, gateway_port) == 1:
            return True, 'Done'

    # -> Tunelling client ack
    tune_cli_ack = knxnet.create_frame(
        knxnet.ServiceTypeDescriptor.TUNNELLING_ACK,
        tune_ack_resp.channel_id,
        0x0,
        tune_req_resp.sequence_counter
    )
    logger.debug('>>> TUNNELLING_ACK\n{}'.format(tune_cli_ack))
    sock.sendto(tune_cli_ack.frame, (gateway_ip, gateway_port))

    # In case of read :
    if payload[2] == 0x0:
        # <- Tunneling request reply
        logger.debug('waiting for a tunneling read reply (TUNNELLING_REQUEST -> tune_read_repl)')
        data_recv, addr = sock.recvfrom(buf_size)
        tune_read_repl = knxnet.decode_frame(data_recv)
        print('Data read : \n\n{}'.format(tune_read_repl))

    # Final disconnect
    disconnect_gateway(tune_ack_resp.channel_id, control_endpoint, sock, gateway_ip, gateway_port)
